...
function register($fullname, $username, $password, $email) {
    ...
    if ($stmt->rowCount() > 0) {
        ...
        $default_profile_pic = 'images/default-profile-pic.jpg';

        // Insert user details into the database
        $query = "INSERT INTO users (fullname, username, password, email, profile_pic) VALUES (:fullname, :username, :password, :email, :profile_pic)";
        $stmt = $conn->prepare($query);
        $stmt->bindValue(':fullname', $fullname);
        $stmt->bindValue(':username', $username);
        $stmt->bindValue(':password', $hashedPassword);
        $stmt->bindValue(':email', $email);
        $stmt->bindValue(':profile_pic', $default_profile_pic);
